...
@section('pageName',trans('web.teamReviews'))
@section('thead')
    <tr>
        <th>{{trans('web.id')}}</th>
        ...
        <th>{{trans('web.teamMemberName')}}</th>
...
@section('modal')
    @include('admin.teamReviews.create',['id'=>'createmodal','name'=>trans('web.createReview'),'action'=>url()->current()])
@stop
@endif
        ...
        $(document).ready(function () {
            ...
            var dataListView = $(".data-list-view").DataTable({
                ...
                buttons: [
                        @if (auth()->User()->group->categories_add == 1)
                    {
                        text: "<i class='feather icon-plus'></i>{{trans('web.createTeamReviews')}}",
                        action: function () {
                ...
                "columns": [
                    ...
                    {data: 'salon_name', name: 'salon_name'},
                    {
                        "mRender": function (data, type, row) {
                            return $("<div/>").html(row.team_rate).text();
                        }
                    },
